...
echo "process:ID
location:ID:init1{initial:}"

for id in `seq 2 1 $N 2> /dev/null`; do
    echo "location:ID:init${id}"
done
...
for id in `seq 1 1 $((N-1)) 2> /dev/null`; do
    echo "edge:ID:init${id}:init$((id+1)):init${id}"
done
echo "edge:ID:init${N}:id0:init${N}"
...
sync:P${pid}@init${pid}:ID@init${pid}"
done
echo ""
