...
namespace tchecker {
  ...
  namespace dbm {
    ...
    /* IMPLEMENTATION NOTES:
     Difference bound #c is encoded in a 32-bits integer with the least-significant bit used to store
     the comparator #: LE (<=) or LT (<), and the other bits used to store c.
    ...
    inline tchecker::dbm::db_t sum(tchecker::dbm::db_t db1, tchecker::dbm::db_t db2)
    {
      ...
      return tchecker::dbm::db(((db1 & tchecker::dbm::LE) && (db2 & tchecker::dbm::LE))
                               ? tchecker::dbm::LE : tchecker::dbm::LT, (db1 >> 1) + (db2 >> 1));
#endif // DBM_UNSAFE
    }
