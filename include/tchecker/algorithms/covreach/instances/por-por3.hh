...
#ifndef TCHECKER_ALGORITHMS_COVREACH_POR_POR3_HH
#define TCHECKER_ALGORITHMS_COVREACH_POR_POR3_HH
...
#include "tchecker/por/por3/builder.hh"
#include "tchecker/por/por3/output.hh"
#include "tchecker/por/por3/state.hh"
#include "tchecker/por/por3/ts.hh"

/*!
 \file por-por3.hh
 \brief Instantiation of covreach algorithm for asynchronous zone graph with
 por3 partial-order reduction
 */
          ...
          namespace async_zg {
            ...
            namespace ta {
              ...
              /*!
               ...
               \brief Model for covering reachability asynchronous zone graphs of por3 timed automata with POR
               */
              ...
              public:
                ...
                using ts_t = tchecker::por::por3::ts_t<typename zone_semantics_t::ts_t>;
                ...
                using builder_t = tchecker::por::por3::states_builder_t<ts_t, ts_allocator_t>;
                ...
                public:
                  using node_ptr_t
                  = typename tchecker::covreach::details::por::por3::async_zg::ta::
                  ...
                  bool operator() (node_ptr_t const & n1, node_ptr_t const & n2)
                  {
                    return ((static_cast<tchecker::ta::state_t const &>(*n1)
                            ...
                            &&
                            tchecker::por::por3::cover_leq(*n1, *n2, _local_enabled)
                            );
                  }
                private:
                ...
                public:
                  bool operator() (node_ptr_t const & n1, node_ptr_t const & n2) const
                  {
                    ...
                    if (cmp > 0)
                      return false;
                    return tchecker::por::por3::lexical_cmp(*n1, *n2) < 0;
                  }
                };

                static std::tuple<tchecker::event_map_t> state_predicate_args(model_t const & model)
                {
                  return std::tuple<tchecker::event_map_t>(tchecker::local_map(model.system()));
                }
                ...
                = tchecker::por::por3::state_outputter_t
            ...
            namespace sync_zones {
              ...
              namespace ta {
                ...
                /*!
                 \class algorithm_model_t
                 \brief Model for covering reachability over por3 timed automata with POR
                 */
                template <class ZONE_SEMANTICS>
                class algorithm_model_t {
                public:
                ...
                public:
                  using node_ptr_t
                  = typename tchecker::covreach::details::por::por3::async_zg::sync_zones::ta::algorithm_model_t<ZONE_SEMANTICS>::node_ptr_t;
            ...
            namespace bounded_spread {
              ...
              namespace ta {
                ...
                /*!
                 ...
                 of por3 timed automata with POR
                 */
                template <class ZONE_SEMANTICS>
                class algorithm_model_t {
                public:
                  ...
                  public:
                    using node_ptr_t
                    = typename tchecker::covreach::details::por::por3::async_zg::bounded_spread::ta::
                      algorithm_model_t<ZONE_SEMANTICS>::node_ptr_t;
...
#endif // TCHECKER_ALGORITHMS_COVREACH_POR_POR3_HH
